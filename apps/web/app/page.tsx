    ...
    <main className="min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      <nav className="fixed top-0 inset-x-0 z-50 border-b border-[var(--border)] bg-[var(--bg)]/90 backdrop-blur-md">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <ThemeToggle />
            <Link href="/login" className="text-sm text-[var(--muted)] hover:text-[var(--text)]">Log in</Link>
            <Link
              href="/signup"
      ...
      <section className="pt-40 pb-24 px-6 text-center">
        ...
        <div className="flex items-center justify-center gap-2 mt-12 flex-wrap">
          <span className="text-[var(--faint)] text-sm">Coverage:</span>
          {LEVELS.map((l) => (
            <span key={l} className="px-3 py-1 rounded-full border border-[var(--border)] text-xs font-mono text-[var(--muted)]">
              {l}
            </span>
          ))}
        </div>
      ...
      <section className="py-20 px-6 bg-[var(--surface)]">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid gap-4">
            {PROBLEMS.map((p) => (
              <div key={p.app} className="flex items-start gap-4 p-5 rounded-xl border border-[var(--border)] bg-[var(--bg)]">
                <span className="text-red-400 text-xl mt-0.5">✗</span>
                <div>
                  <span className="font-semibold text-[var(--text)]">{p.app}: </span>
                  <span className="text-[var(--muted)]">{p.problem}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <section id="how-it-works" className="py-24 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-8 mb-6">
            <p className="text-[var(--faint)] text-xs uppercase tracking-widest mb-4 font-medium">
            ...
            <div className="mb-6">
              <p className="text-2xl font-bold mb-1">
                ...
                <span className="text-[var(--text)]">habe</span>{' '}
                <span className="text-orange-400">einen Bruder</span>{' '}
                <span className="text-[var(--text)]">und</span>{' '}
                <span className="text-pink-400">eine Schwester</span>.
              </p>
              <p className="text-[var(--muted)] text-sm italic">I have a brother and a sister.</p>
            </div>

            <div className="grid sm:grid-cols-3 gap-3 mb-8">
              <div className="rounded-lg border border-blue-500/20 bg-blue-500/5 p-3">
                <p className="text-blue-400 text-xs font-semibold uppercase tracking-wide mb-1">Nominative · Subject</p>
                <p className="text-[var(--text)] font-mono text-sm">Ich</p>
                <p className="text-[var(--faint)] text-xs">The one doing the action</p>
              </div>
              <div className="rounded-lg border border-orange-500/20 bg-orange-500/5 p-3">
                <p className="text-orange-400 text-xs font-semibold uppercase tracking-wide mb-1">Accusative · Direct Object</p>
                <p className="text-[var(--text)] font-mono text-sm">einen Bruder</p>
                <p className="text-[var(--faint)] text-xs">Masculine → ein becomes einen</p>
              </div>
              <div className="rounded-lg border border-pink-500/20 bg-pink-500/5 p-3">
                ...
                <p className="text-[var(--text)] font-mono text-sm">eine Schwester</p>
                <p className="text-[var(--faint)] text-xs">Feminine → eine stays eine</p>
              </div>
            </div>

            <div className="rounded-xl border border-[#d4a843]/20 bg-[#d4a843]/5 p-5">
              <p className="text-[var(--gold)] text-xs font-semibold uppercase tracking-wide mb-2">
              ...
              <p className="text-[var(--text-soft)] text-sm leading-relaxed">
                ...
                <span className="text-[var(--text)] font-mono">ein → einen</span>.
          ...
          <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-8">
            <p className="text-[var(--faint)] text-xs uppercase tracking-widest mb-6 font-medium">
              Quick Reference · Nominative vs Accusative
            </p>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-[var(--faint)] text-xs uppercase tracking-wider">
            ...
            <p className="text-[var(--faint)] text-xs mt-4">← Only masculine changes in Accusative. Everything else stays the same.</p>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {FEATURES.map((f) => (
              <div
                key={f.title}
                className="p-6 rounded-2xl border border-[var(--border)] bg-[var(--bg)] hover:border-[var(--border)] transition-colors"
              >
      ...
      <section className="py-24 px-6">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-[var(--muted)] text-center mb-12">The features that actually matter for real German fluency.</p>
          <div className="rounded-2xl border border-[var(--border)] overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--border)]">
                  <th className="text-left p-4 font-medium text-[var(--faint)]">Feature</th>
                  <th className="p-4 text-center font-bold text-[var(--gold)]">Sprich</th>
                  <th className="p-4 text-center font-medium text-[var(--faint)]">Duolingo</th>
                  <th className="p-4 text-center font-medium text-[var(--faint)]">Babbel</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5">
                {COMPARISON.map((row) => (
                  <tr key={row.feature} className="hover:bg-white/[0.02] transition-colors">
                    <td className="p-4 text-[var(--text-soft)]">{row.feature}</td>
                    <td className="p-4 text-center text-lg">{row.sprich ? <span className="text-green-400">✓</span> : <span className="text-[var(--faint-2)]">✗</span>}</td>
                    <td className="p-4 text-center text-lg">{row.duolingo ? <span className="text-green-400">✓</span> : <span className="text-[var(--faint-2)]">✗</span>}</td>
                    <td className="p-4 text-center text-lg">{row.babbel ? <span className="text-green-400">✓</span> : <span className="text-[var(--faint-2)]">✗</span>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="py-24 px-6 text-center">
        <div className="max-w-2xl mx-auto">
          <h2 className="text-4xl md:text-5xl font-black mb-4">
            Sprich.<br />
            <span className="text-[var(--faint)]">German for people who mean it.</span>
          </h2>
          <p className="text-[var(--muted)] mb-10">Free to start. No credit card. No guilt streaks. Just real German learning.</p>
          <Link
            href="/signup"
      ...
      <footer className="border-t border-[var(--border)] py-8 px-6">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <p className="text-[var(--faint)] text-sm">© {new Date().getFullYear()} Sprich. Built for every Pakistani going to Germany.</p>
